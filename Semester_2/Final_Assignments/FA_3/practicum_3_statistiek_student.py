...
def median(lst):
    ...
    return float(sorted(lst)[((len(lst) - 1) // 2)]) if len(lst) % 2 else float(
        (sorted(lst)[((len(lst) - 1) // 2)] + sorted(lst)[((len(lst) - 1) // 2) + 1]) / 2)
...
def q3(lst):
    ...
    return median(sorted(lst)[len(lst) // 2+(len(lst) %2):])


def var(lst):
    """ Retourneer de variantie (float) van de lijst lst. """
    # De map functie in om de function irerable te maken, anders krijg je een TypeError: 'function' object is not iterable
    return sum(map(lambda i: (i-mean(lst))**2, lst)) / len(lst)
...
def modes(lst):
    ...
    modi = set(filter(lambda i: lst.count(i) == max(map(lst.count, lst)) if lst else sorted(lst), lst))
    return list(modi) if set(lst) - modi else sorted(lst)
