...
def linear_search_recursive(lst, target):
    ...
    if n < 0:
        return False
    return True if lst[n] == target else linear_search_recursive(lst[:n], target)
...
def binary_search_recursive(lst, target):
    ...
    q = len(lst) // 2
    if len(lst) == 1:
        return True if lst[q] == target else False
    elif lst[q] == target:
        return True
    else:
        if lst[q] < target:
            return binary_search_recursive((lst[q:]), target)
        else:
            return binary_search_recursive((lst[:q]), target)
